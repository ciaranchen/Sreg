...
def check(plugin, passport, passport_type):
    ...
    if plugin['request']['method'] == "GET":
        try:
            ...
            s = requests.Session()
            s.headers = headers
            content = s.get(url, headers={}, timeout=8).text
        ...
        try:
            s = requests.Session()
            s.headers = headers
            content = s.post(url, data=post_data, headers={}, timeout=8).text
        except Exception as e:
            print(inRed('\n[-] %s Error: %s\n' % (app_name, str(e))))
            return
    ...
    for plugin in plugins:
        ...
        if parser_argument.cellphone:
            p = multiprocessing.Process(target=check,
                                        args=(content, str(parser_argument.cellphone), "cellphone"))
        elif parser_argument.user:
            p = multiprocessing.Process(target=check,
                                        args=(content, str(parser_argument.user), "user"))
        elif parser_argument.email:
            p = multiprocessing.Process(target=check,
                                        args=(content, str(parser_argument.email), "email"))
        p.start()
        jobs.append(p)
